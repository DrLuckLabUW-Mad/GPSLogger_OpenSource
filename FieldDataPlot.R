...
#Make sure to go to Tools>Global Options>Default working directly (when not in a project)
#>Browse and select the folder where the logs are located
fielddata <- read.csv("FILE NAME.csv", header = T)
...
FieldCoord <- make_bbox(lon = fielddata$longitude, lat = fielddata$latitude, f = 1)
FieldCoord
